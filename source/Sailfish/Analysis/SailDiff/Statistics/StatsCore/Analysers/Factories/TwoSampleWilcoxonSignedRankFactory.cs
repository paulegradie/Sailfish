...
internal static class TwoSampleWilcoxonSignedRankFactory
{
    ...
    {
        if (sample1.Count != sample2.Length) throw new DimensionMismatchException(nameof(sample2), "Both samples should be of the same size.");
        ...
        if (ranks.Length == 0) throw new ArgumentException($"The {nameof(TwoSampleWilcoxonSignedRank)} must be provided valid samples");
