...
public static class StatsOps
{
    ...
    {
        for (long i = 0; i < n; ++i) yield return i;
    }
    ...
    public static T[] Get<T>(this T[] source, int startRow, int endRow)
    {
        ...
        for (var index = startRow; index < endRow; ++index) objArray[index - startRow] = source[index];
    ...
    public static T[] Concatenate<T>(this T[] a, params T[] b)
    {
        ...
        for (var index = 0; index < a.Length; ++index) objArray[index] = a[index];

        for (var index = 0; index < b.Length; ++index) objArray[index + a.Length] = b[index];
