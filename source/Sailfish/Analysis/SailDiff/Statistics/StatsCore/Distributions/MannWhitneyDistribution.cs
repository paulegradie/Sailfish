...
public class MannWhitneyDistribution : UnivariateContinuousDistribution
{
    ...
    protected internal override double InnerInverseDistributionFunction(double p)
    {
        ...
        var num1 = 0.0;
        var num2 = 0.0;
        var num3 = DistributionFunction(0.0);
        if (num3 > p)
        {
            for (; num3 > p && !double.IsInfinity(num1); num3 = DistributionFunction(num2))
            {
                num1 = num2;
                num2 = 2.0 * num2 + 1.0;
            }
        ...
        else
        {
            if (num3 >= p)
                return 0.0;
            for (; num3 < p && !double.IsInfinity(num2); num3 = DistributionFunction(num1))
            {
                num2 = num1;
                num1 = 2.0 * num1 - 1.0;
            }
