...
When enabled, scalefish will discover test cases with scalefish-enabled variables and fit the timing data against a catalog of complexity curves (Constant, Logarithmic, SqrtN, Linear, NLogN, Quadratic, Cubic, Exponential, Factorial). It ranks the candidates using the **corrected Akaike Information Criterion (AICc)** — computed from the per-X replicate spread whenever replicate data is available, so even a 3-value declaration can produce a confident classification — and reports a confidence-aware classification with a continuous-exponent diagnostic.
